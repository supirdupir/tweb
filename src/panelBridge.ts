...
class PanelBridge implements PanelBridgeAPI {
  ...
  async getTwoFAPassword(): Promise<string | null> {
    ...
    try {
      ...
      throw new PanelBridgeError('network', `getTwoFAPassword network error: ${(e as Error).message}`);
    }
    if(!res.ok) {
      throw new PanelBridgeError(
        mapStatusToCode(res.status),
  ...
  async onSessionSaved(snapshot: PanelSessionSnapshot): Promise<void> {
    ...
    try {
      ...
      throw new PanelBridgeError('network', `onSessionSaved network error: ${(e as Error).message}`);
    }
    if(!res.ok) {
      throw new PanelBridgeError(
        mapStatusToCode(res.status),
  ...
  async restoreCachedSession(): Promise<RestoreResult> {
    console.warn('[panelBridge] restoreCachedSession begin at', Date.now(), 'cached=', this.cached, 'accountId=', this.accountId);
    if(!this.cached) {
  ...
  private async _seedTwebAuthState(snap: {authKeyHex: string, dcId: number, userId: number}): Promise<void> {
...
async function clearTwebLegacyCacheStorage(): Promise<void> {
  ...
  await Promise.all(CACHE_STORAGE_DB_NAMES.map(async(name) => {
    try {
...
export function initPanelBridge(): InitPanelBridgeResult {
  ...
  console.log('[panelBridge] init:',
    'accountId=', getPanelAccountId(),
    'self.name=', (self as any).name,
    'href=', location.href);
  ...
  const origin: string = (import.meta.env && (import.meta.env as any).VITE_PANEL_ORIGIN) || 'http://localhost:8000';
  ...
  console.warn('[panelBridge] DeferredIsUsingPasscode resolved at', Date.now(), 'accountId=', accountId);
